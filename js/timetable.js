...
export class Timetable {
    ...
    renderCell(cell, p, d) {
        ...
        if (this.isEditing) {
            return `
        <td class="border-b border-slate-200 p-1 text-center transition-colors" style="${bgStyle}">
          <div class="flex flex-col gap-1">
            <input type="text" class="w-full text-center border border-slate-300 rounded px-1 py-0.5 text-sm focus:border-primary-500 outline-none bg-white/80"
      ...
      <td class="border-b border-slate-200 p-1 text-center transition-colors" style="${bgStyle}">
        <div class="min-h-[3rem] flex flex-col items-center justify-center px-1">
          <div class="font-semibold text-slate-800 text-sm ${!cell.subject ? 'text-slate-300' : ''}">${escapeHTML(cell.subject) || '　'}</div>
          ${cell.memo ? `<div class="text-[10px] text-slate-500 mt-0.5">${escapeHTML(cell.memo)}</div>` : ''}
        </div>
      </td>`;
    }
    ...
    render() {
        ...
        const headerRow = `<tr>
      <th class="border-b border-r border-slate-200 p-2 text-center text-slate-400 font-medium bg-slate-50/50 w-20 text-xs">${showTimes ? '時限/時間' : ''}</th>
      ${days.map(d => `<th class="border-b border-slate-200 p-2 text-center text-slate-600 font-semibold bg-slate-50/50">${d}</th>`).join('')}
        ...
        this.container.innerHTML = `<table class="w-full text-sm text-left border-collapse min-w-[500px]">
