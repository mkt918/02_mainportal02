export class Calendar {
  ...
  // 直近のメモ（今日以降、最大3件）を取得
  getUpcomingNotes(maxCount = 3) {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    return Object.entries(this.notes)
      .map(([key, text]) => ({ key, text, date: new Date(key + 'T00:00:00') }))
  ...
  renderNotePanel(key) {
    ...
    panel.innerHTML = `
      <div class="mt-3 pt-3 border-t border-slate-100">
        ...
        <textarea id="cal-note-input" class="w-full text-sm border border-slate-200 rounded-lg p-2 h-20 resize-none focus:border-primary-500 outline-none" placeholder="メモを入力...">${existing}</textarea>
  ...
  renderUpcomingNotes() {
    ...
    if (!upcoming.length) {
      area.innerHTML = '<p class="text-xs text-slate-400 italic">直近のメモはありません</p>';
      return;
    }
  ...
  render() {
    ...
    html += `</div>`;
    html += `<p class="text-[10px] text-slate-400 mt-2 text-center">日付をクリックしてメモを追加</p>`;
